...
var EventBase = ValidationBase.extend({
   ...
   onCascadeRemove: function(resolve, reject) {
      ...
      if (this.cascade_remove) {
         ...
         realm.each(self.cascade_remove, function(path) {
            if (!_.isString(path)) {
               return reject({
                  status: 500,
            ...
            if (!match) {
               ...
               // Require model
               return realm.require(model, function(Instance) {
                  ...
                  if (action === "exclude") {
                     ...
                     return Instance.find(criteria).all().then(function(_records) {
                        return realm.each(_records, function(record) {
                           //1
                  ...
                  if (action === "nullify") {
                     ...
                     return Instance.find(criteria).all().then(function(_records) {
                        ...
                        return realm.each(_records, function(_record) {
...
var DBRequest = Query.extend({
   ...
   createAllIndexes: function() {
      ...
      return realm.each(indexes, function(index) {
   ...
   resolveWithRequest: function(resolve, reject) {
      ...
      if (!opts.target._wires_mongo_model) {
         ...
         if (_.isFunction(opts.target)) {
            return realm.each(models, function(model) {
...
Model = AccessHelpers.extend({
   ...
   add: function(target, property) {
      var self = this;
      if (_.isArray(target)) {
         return realm.each(target, function(item) {
...
realm.module('realm.mongo.Model', function() {
   return Model;
});
realm.service("$wiresMongoIndexer", function() {
   return function() {
      var _models = _.flatten(arguments);
      logger.info("Requiring %s models", _models.length);
      return realm.each(_models, function(modelPath) {
         logger.info("Requiring %s", modelPath);
         return new Promise(function(resolve, reject) {
            realm.require(modelPath, function(modelInstance) {
         ...
         return realm.each(models, function(ModelClass) {
